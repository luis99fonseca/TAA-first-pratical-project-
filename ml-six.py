...
## RBF Tunning
kernels = ['poly']
Css = [0.3, 1, 3, 7, 10, 30] # [0.01, 0.03, 0.1,
gammas = [0.01, 0.1, 0.3, 1, 3] # 0.03 30 10

with open("resultsPoly.txt", "w") as file01:
# if True:
    if True:
        for c in Css:
            coords1 = []
            ...
            # if True:
                for k in kernels:
                    for d in range(3, 10):
                        ...
                        # sys.exit(-1)
                        logreg = svm.SVC(C=c, kernel=k, degree=d, gamma=g)
                        ...
                        # )
                        temp_obj = logreg.fit(x_train_rolled, y_train_rolled)
                        file01.write("---------- c=" + str(c) + "; g=" + str(g) + "; k=" + str(k) + "; d=" + str(d) + "\n")
